...
class AlipayDirectController extends Controller
{
    ...
    public function __construct(){
        ...
        if(!$rd){
            $rd = D("order")->where(array("order_sn"=>I('get.order_sn')))->getField("restaurant_id");
...
//        $alipay_config['notify_url'] = 'http://shop.founpad.com/index.php/home/AlipayDirect/notify';
        $alipay_config['notify_url'] = "http://".$_SERVER["HTTP_HOST"].'/index.php/home/AlipayDirect/notify';
...
//                session("num".$order_sn,1);
    ...
    function alipay_code(){
        ...
        if(!$restaurant_id){
            $restaurant_id = D("order")->where(array("order_sn"=>I('get.order_sn')))->getField("restaurant_id");
		...
		if (!empty($out_trade_no)&& trim($out_trade_no)!=""){
            ...
            // 避免两台机器同时下单
            $order_num = D("order")->where($o_condition)->count();
            if($order_num>1){
                echo '此笔订单重复，请重新点餐';
                exit;
            }
