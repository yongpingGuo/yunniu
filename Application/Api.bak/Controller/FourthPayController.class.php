...
class FourthPayController extends BaseController
{
    ...
    public function pay_scan()
    {
        ...
        if ($this->is_security) {
            ...
            // 将民生订单号关联到数据库订单号
            $res = D('order')->where(array('order_sn'=>$order_sn))->save(array('minsheng_post_no'=>$fourth_sn));
            if($res === false){
                $returnData['code'] = 0;
            ...
            // 订单金额
//            $total_aomount = D('order')->where(array('order_sn'=>$order_sn))->getField('total_amount');

            // 订单信息
            $order_info = D('order')->where(array('order_sn'=>$order_sn))->field('total_amount,order_status')->find();
            if(empty($order_info)){
                $returnData['code'] = 0;
...
//            file_put_contents(__DIR__."/"."check_sign_fourth.txt",'weixin_zhusao:'.$return.",time:".date("Y-m-d H:i:s",time())."\r\n\r\n",FILE_APPEND);
    ...
    public function check_sign($returnText){
        ...
        $restaurant_id = D("order")->where(array('minsheng_post_no'=>$minsheng_post_no))->getField('restaurant_id');
        $MD5_KEY = D('fourth_md5_key')->where(array('restaurant_id'=>$restaurant_id))->getField('md5_key');
        if($MD5_KEY == null){
            return false;
        }
