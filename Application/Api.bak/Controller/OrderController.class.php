...
class orderController extends BaseController
{
    ...
    */
    public function getInfo() {
        ...
        if($this->is_security) {
            $order_sn = I("order_sn");
            if(empty($order_sn)) $this->ajaxReturn(array('code'=>'1', 'msg'=> '订单编号不能为空'));
    ...
    */
    public function updatInfo() {
        $this->validates();
        $order_sn = I("order_sn");
        if(empty($order_sn)) $this->returnJson('1', '订单编号不能为空');
        $S_Order = new ServiceOrder();
        $where['order_sn'] = $order_sn;
        $where['restaurant_id'] = session("restaurant_id");
        $res = $S_Order->updateInfo($where, array('order_status'=>1));
        if($res) $this->returnJson("1", '修改成功');
        $this->returnJson("1", '修改失败');
    }
    # 原来的收银端开始
...
// //                            dump($sum);
    ...
    public function cash_pay_only()
    {
        ...
        if ($this->is_security) {
            ...
            $order_info = D("order")->where(array('order_sn'=>$client_order))->find();
...
//                                $food_price2+=$food_attribute_info['attribute_price'];
                ...
                */
                ...
                if($orderDataInfo->if_enjoy_benefits == 1){
                    /*file_put_contents(__DIR__."/"."cash_pay_only_restaurant_discount.txt",'是否享受优惠为：'. $orderDataInfo->if_enjoy_benefits.
                        '，订单号为：'. $client_order.
