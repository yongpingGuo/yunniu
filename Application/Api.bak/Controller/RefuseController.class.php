...
class RefuseController extends BaseController
{
    ...
    public function get_order(){
      ...
      if($this->is_security && $where['restaurant_id']){
        ...
        $order_list=M('order')->where(array($where))->field('order_id,order_sn,add_time,total_amount,zhifuhao,take_num,pay_type,refuse')->order('order_id desc')->select();
        if($order_list){
          foreach ($order_list as $key => $value) {
            $order_list[$key]['food_list']=M('order_food')->join('left join food on order_food.food_id=food.food_id')
                                                          ->join('left join printer on food.print_id=printer.printer_id')
                                                          ->where(array('order_id'=>$value['order_id']))
                                                          ->field('order_food_id,order_food.food_num,order_food.food_price2,order_food.food_name,refuse_num,printer_ip')
                                                          ->select();
            foreach ($order_list[$key]['food_list'] as $k => $val) {
              $order_list[$key]['food_list'][$k]['food_attribute']=M('order_food_attribute')->join('left join printer on order_food_attribute.print_id=printer.printer_id')->where(array('order_food_id'=>$val['order_food_id']))->field('food_attribute_name,food_attribute_price,printer_ip')->select();
            }
          }
    ...
    public function refuse(){
      ...
      if($this->is_security && $data['restaurant_id']){
        ...
        $order_info=M('order')->where(array('order_id'=>$order_id,'order_sn'=>$order_sn,'refuse'=>'0','order_status'=>array('not in','0,1,2')))->field('order_id,refuse')->find();
        if($refuse_type=='1' && $order_info['refuse']=='0'){
          $refuse_all=M('order')->where(array('order_id'=>$order_id,'order_sn'=>$order_sn,'order_status'=>array('not in','0,1,2')))->data(array('refuse'=>'1','refuse_reason'=>$reason))->save();
          if($refuse_all){
          ...
          if($order_info['refuse']=='0'){
            $refuse_food=$model->table('order')->where(array('order_sn'=>$order_sn,'refuse'=>'0','order_status'=>array('not in','0,1,2')))->data(array('refuse'=>'2'))->save();
            if(!$refuse_food){
          ...
          $order_food_info=$model->table('order_food')->where(array('order_id'=>$order_id))->field('order_food_id,food_num,refuse_num')->select();
          $order_food_id=0;
            ...
            foreach ($order_food_info as $k => $val) {
              if($value['order_food_id']==$val['order_food_id']){
                if($val['food_num']>=($val['refuse_num']+$value['refuse_num'])){
                  ...
                  $up_order_foods=$model->table('order_food')->where(array('order_id'=>$order_id,'order_food_id'=>$value['order_food_id']))->data(array('refuse_num'=>($val['refuse_num']+$value['refuse_num']),'refuse_reason'=>$reason))->save();
                  if(!$up_order_foods){
                    $model->rollback();
                    $this->ajaxReturn(array('code'=>'0','msg'=>'菜品退单失败'));
                  }
    ...
    public function get_result_order_info($order_sn,$order_food_id=''){
      $order_info=M('order')->where(array('order_sn'=>$order_sn))->field('order_id,order_sn,add_time,total_amount,zhifuhao,take_num,pay_type,refuse,refuse_reason')->find();
      if($order_info){
        $where['order_id']=$order_info['order_id'];
        if($order_food_id!=''){
          $where['order_food_id']=$order_food_id;
        }
        $order_info['food_list']=M('order_food')->join('left join food on order_food.food_id=food.food_id')
                                                      ->join('left join printer on food.print_id=printer.printer_id')
                                                      ->where($where)
                                                      ->field('order_food_id,order_food.food_num,order_food.food_price2,order_food.food_name,refuse_num,refuse_reason,printer_ip')
                                                      ->select();
        foreach ($order_info['food_list'] as $k => $val) {
          $order_info['food_list'][$k]['food_attribute']=M('order_food_attribute')->join('left join printer on order_food_attribute.print_id=printer.printer_id')->where(array('order_food_id'=>$val['order_food_id']))->field('food_attribute_name,food_attribute_price,printer_ip')->select();
        }
      }
      return $order_info;
    }
