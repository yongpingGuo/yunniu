...
class QiShouFourthPayController extends BaseController
{
    /**
     * 同步md5秘钥
     */
...
//            file_put_contents(__DIR__."/"."zhusao_code.txt",'weixin_receive:'.$returnText_weixin.",time:".date("Y-m-d H:i:s",time()).",session_id:".session('restaurant_id')."\r\n\r\n",FILE_APPEND);
    ...
    public function pay_code_in_place_order($data_arr)
    {
        ...
        if ($this->is_security) {
            ...
            // 将民生订单号关联到数据库订单号
            $qs_order = M('qs_order');
            $res = $qs_order->where(array('order_sn'=>$order_sn))->save(array('minsheng_post_no'=>$fourth_sn));
            if($res === false){
                $returnData['code'] = 0;
            ...
            // 订单信息
            $order_info = $qs_order->where(array('order_sn'=>$order_sn))->field('total_price,order_status')->find();
            if(empty($order_info)){
                $returnData['code'] = 0;
                $returnData['msg'] = '没有对应的订单信息';
                return $returnData;
            ...
            $total_aomount = $order_info['total_price'];

            // 请求支付宝二维码
            $ali_notity_url = "http://".$_SERVER["HTTP_HOST"]."/index.php/Api/QiShouFourthPay/ali_notify";
            $APIurl = 'http://payapi.498.net/api3.0/api.php'; //接口地址
            ...
            // 请求微信二维码
            $weixin_notity_url = "http://".$_SERVER["HTTP_HOST"]."/index.php/Api/QiShouFourthPay/weixin_notify";
            $APIurl = 'http://payapi.498.net/api3.0/api.php'; //接口地址
            $weixin_data['a'] = "payCode";
            $weixin_data['i'] = $operater_id;
            $weixin_data['j'] = $total_aomount;
...
//            file_put_contents(__DIR__."/"."zhusao_code.txt",'weixin_receive:'.$returnText_weixin.",time:".date("Y-m-d H:i:s",time()).",session_id:".session('restaurant_id')."\r\n\r\n",FILE_APPEND);
    ...
    public function is_success($returnText){
        ...
        if($f == 1 && $m == "支付成功!"){
            ...
            $qs_order = M('qs_order');
            $res = $qs_order->where($where)->save($save);
            if($res === false){
                return false;
            }
            // 删除第四方支付二维码
    ...
    public function check_sign($returnText){
        ...
        $restaurant_id = M('qs_order')->where(array('minsheng_post_no'=>$minsheng_post_no))->getField('restaurant_id');
        $MD5_KEY = D('fourth_md5_key')->where(array('restaurant_id'=>$restaurant_id))->getField('md5_key');
        if($MD5_KEY == null){
            return false;
        }
