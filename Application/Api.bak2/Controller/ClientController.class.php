...
//        $device_code = I("post.device_code");
    ...
    public function restaurantAddress()
    {
        ...
        if ($this->is_security) {
            ...
            $info["top_logo_url"] = C('HOST_NAME') . $restaurant_info['top_logo'];        //上logo
            $info["next_logo_url"] = C('HOST_NAME') . $restaurant_info['next_logo'];        //下logo

            // 打印机小票内容是否要打印
            $restaurant_bill = D("restaurant_bill");
            $restaurant_bill_info = $restaurant_bill->where($p_condition)->find();
            if (empty($restaurant_info['top_logo'])) {
                //店铺没有上传logo则小票不打印logo
                $restaurant_bill_info['top_logo'] = 0;
            }
            if (empty($restaurant_info['next_logo'])) {
                //店铺没有上传logo则小票不打印logo
                $restaurant_bill_info['next_logo'] = 0;
            }
    ...
    public function vip_score_cancel()
    {
        ...
        if ($this->is_security) {
            ...
            // 判断是代理的会员还是店铺的会员
            if ($order_goods_info['restaurant_or_business'] == 1) {
                // 代理会员
                // 查出当前店铺所属的代理ID
...
//                $score_where['business_id'] = $now_business_id;
            } else {
                // 店铺会员
                if ($order_goods_info['restaurant_id'] != session('restaurant_id')) {
                    $content["type"] = "balance";
                    $content["code"] = "0";
                    $content["order_sn"] = $qr_number;
                    $content["msg"] = "您不是当前店铺的会员";
...
//                $score_where['restaurant_id'] = session('restaurant_id');
            ...
            $score_where['restaurant_id'] = session('restaurant_id');
            $score_where['type'] = 4;
            $if_open = M("set")->where($score_where)->getField("if_open");
            // 判断是否开启
            if ($if_open) {
                ...
                if ($order_goods_info['score'] > $vip_info['score']) {
                    ...
                    $score_goods_order_data['summary_score'] = $vip_info['score'] - $order_goods_info['score']; // 此时的会员总分
...
//                $vip_info      = $vip_model->where($blc_where)->field("id,username,remainder,score,total_consume,business_id")->find();
                $vip_info = $vip_model->where($blc_where)->field("id,username,remainder,score,total_consume,business_id,restaurant_or_business,restaurant_id")->find();
...
//            $order_info = order()->where(array('order_sn'=>$order_sn))->field('order_id,order_status')->find();
            ...
            if ($order_info['order_status'] == 3) {
                ...
                // 删除order_food表数据
                $res1 = order_F()->where(array('order_id' => $order_info['order_id']))->delete();
