...
//                'type'=>1
    ...
    public function getOrder()
    {
        ...
        if ($this->is_security) {
            ...
            $where['order_status'] = 3;
            $where['restaurant_id'] = session('restaurant_id');
            $where['push_status'] = array('in','1,3');  // 未未完成的订单
            $orderInfo = order()->where($where)->field('order_status,order_sn,take_num,push_status,cancell_num')->select();
            $returnData['code']     = 1;
            $returnData['info']     = $orderInfo;
            $returnData['msg']      = "获取数据成功";
            exit(json_encode($returnData));
...
//                'type'=>1
    ...
    public function cupboard_qc()
    {
        ...
        $occupy_where['type'] = 1;
        $group_id = M('dc_window_info')->where($occupy_where)->getField('group_id');
        $d_code = M('dc_window_group')->where(array('group_id'=>$group_id))->getField('device_code');
        ...
        // 更改取餐屏和放餐屏的设备占用情况
        $res2 = M('dc_window_info')->where($occupy_where)->save($save);
        // 更改取餐屏的设备占用情况
    ...
    {
        // 获取核销屏的device_id
        $device_id = M('dc_take_meal_device')->where(array('type'=>2,'restaurant_id'=>131))->field('device_id')->select();
        if(!$device_id){
            $returnData['code']     = 0;
            $returnData['msg']      = "该店铺没有设置好设备";
            exit(json_encode($returnData));
        ...
        $order_sn = 'DC0013118041014440400002';
        $type = 'put_meal';  // 放餐屏放餐
        $push_data['cancell_num'] = order()
            ->where(array('order_sn'=>$order_sn))
            ->getField('cancell_num');
        $push_data['order_sn'] = $order_sn;
        if($device_id){
            $res = $S_TakeMeal->pushTwoScreen($type,$device_id,$push_data);
        }
    ...
    public function ttt2()
    {
        ...
        $S_TakeMeal = new ServiceTakeMeal();
        $order_sn = 'DC0013118041014440400002';
        $S_TakeMeal->pushOneScreen($order_sn);
    }
