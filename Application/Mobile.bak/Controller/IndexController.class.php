...
//                        exit;
    ...
    */
    public function payConfigLoad($order_sn, $openid) {
        ...
        $S_Restaurant = new Restaurant();
        $restaurant_info = $S_Restaurant->getInfo();
        file_put_contents(__DIR__."/". "restaurant_info.txt", 'restaurant_id:'.session('restaurant_id').'wx_prepaid_flag:'.session("wx_prepaid_flag").'business_id:'.session("business_id"));
        ...
        file_put_contents(__DIR__."/". "jsApiParameters.txt", 'jsApiParameters:'.json_encode($jsApiParameters));
...
//            $url = $jsApi->createOauthUrlForCode("http://".$_SERVER["HTTP_HOST"]."/index.php/mobile/Index/pay_old/order_sn/".$order_sn);
    ...
    public function change_order_type()
    {
        ...
        $res = D('order')->where(array('order_sn'=>$order_sn))->save(array('order_type'=>$order_type));
        if($res !== false){
            $return['code'] = 1;
            $return['msg'] = '成功';
