...
//          I("get.restaurant_id");
    ...
    public function receiver_weixin(){
        ...
        if($vip_info){
            ...
            // 弹出输入手机号的界面，再结合session中，restaurant_flag是否有值来判断当前进来的是店铺还是代理
            /*$arr = array(
                'openid'=>$res3['openid'],
                'headimgurl'=>$res3['headimgurl']
            );*/
            session("headimgurl",$res3['headimgurl']);
            session("nickname",$res3['nickname']);
            session("sex",$res3['sex']);
            $this->inputPhone();
    ...
    public function inputPhone()
    {
        if(IS_POST){
            ...
            if(session("restaurant_flag")){
                // 当前是店铺进来的
                $where['restaurant_or_business'] = 2;
                $where['phone'] = $phone;
                $where['restaurant_id'] = session('restaurant_id');
                $vipInfo = M('vip')->where($where)->find();
                if($vipInfo){
                    // 存在对应的会员信息，补全openid、头像
//                    $save['headimgurl'] = $res3['headimgurl'];
                    $save['headimgurl'] = session('headimgurl');
//                    $save['openid'] = $res3['openid'];
                    $save['openid'] = session('openid');
                    $res = M('vip')->where($where)->save($save);
                    // 进入会员中心
                    $return['destinct'] = 1; // 1代表会员中心
                    $return['info'] = $vipInfo['id']; // 会员id
                    $this->ajaxReturn($return);
//                    $this->vipCenter($vip_info);
                }else{
                    // 没有对应的会员，跳到注册界面
//                    $this->redirectReg($res3,2);
                    $return['destinct'] = 2; // 2代表注册界面
//                    $return['info'] = json_encode($res3); // 会员信息
                    $return['info'] = ''; // 会员信息
                    $return['type'] = 1; // 表示代理
                    $this->ajaxReturn($return);
                }
                ...
                // 当前是代理进来的
                $where['restaurant_or_business'] = 1;
                $where['phone'] = $phone;
...
//                    $save['openid'] = $res3['openid'];
                    ...
                    $res = M('vip')->where($where)->save($save);
                    // 进入会员中心
                    $vip_info = M('vip')->where($where)->find();
                    $return['destinct'] = 1; // 1代表会员中心
                    $return['info'] = $vipInfo['id']; // 会员id
                    $this->ajaxReturn($return);
//                    $this->vipCenter($vip_info);
...
//                    $this->redirectReg($res3,1);
    ...
    public function vipCenter(){
        ...
        if($vip_info['age'] != $real_age){
            // 不相等则进行更新处理
            M('vip')->where(array("openid"=>session("openid")))->save(array("age"=>$real_age));
        }
        // 跳到会员中心
        session("USER",array(
            // 以下两者都能单独唯一标识会员
        ...
        /**********轮播广告************/
        $advtVipModel = M("advertisement_vip");
        $where = [];
        $where['restaurant_id'] = session('restaurant_id');
        $where['advertisement_type'] = 0;
        $info = $advtVipModel->where($where)->select();
...
//        $res3 = $_GET['info'];
        $restaurant_or_business = I('get.type');
        $phone = I('get.phone');
        // 判断session中代理id对应的短信配置表的is_new_user的值，0是旧用户，则使用旧的阿里大于短信接口，1是新用户，使用搬迁后的阿里短信接口
        if($restaurant_or_business == 2){
            // 店铺
            $old_or_new = M("sms_vip_restaurant")->where(array("restaurant_id"=>session("restaurant_id")))->getField("is_new_user");
        }else{
            // 代理
            $old_or_new = M("sms_vip")->where(array("business_id"=>session("business_id")))->getField("is_new_user");
        }
        $this->assign("old_or_new",$old_or_new);
        ...
        $translate = array();
//        $translate['openid'] =  $res3['openid'];
        $translate['openid'] =  session("openid");
//        $translate['nickname'] = $res3['nickname'];
        $translate['nickname'] = session("nickname");
//        $translate['sex'] = $res3['sex'];
        $translate['sex'] = session("sex");
//        $translate['headimgurl'] = $res3['headimgurl'];
        $translate['headimgurl'] = session("headimgurl");
    ...
    public function placeorder(){
        ...
        if(session("restaurant_flag")){
            ...
            $condition2['restaurant_id'] = session('restaurant_id');  //店铺id
        }else{
            // 关联代理
            $condition2['restaurant_or_business;'] = 1;
            $condition2['business_id'] = session('business_id');  //代理id
        }
...
//        $order->where('order_status>=3 AND order_status<>10')->select();
        ...
        for($i = 12;$i>= $current_month;$i--){
            if($i == 12){
                ...
                $end_time = strtotime(($last_year+1)."-1-1 00:00:00")-1;   // 下个月的一号的前一点
            }else{
                $begin_time = strtotime($last_year."-".$i."-1 00:00:00");   // 当前月的一号
            ...
            $yearMonth = "order_".date("Ym",$begin_time);
            $previous_year[$i] = M($yearMonth)->where($condition)->order("pay_time desc")->select();
        }
        ...
        for($j = $current_month;$j>= 1;$j--){
            if($j == 12){
                ...
                $jieshu_time = strtotime(($current_year+1)."-1-1 00:00:00")-1;
            }else{
                //2016-3 开始时间-结束时间
                $kaishi_time = strtotime($current_year."-".$j."-1 00:00:00");
            ...
            $yearMonth = "order_".date("Ym",$kaishi_time);
            $now_year[$j] = M($yearMonth)->where($condition)->order("pay_time desc")->select();
        }
...
//            $id = D("vip")->where(array("openid"=>$openid))->getField("id");
            $vipInfo = M("vip")->where(array("openid"=>$openid))->field("id,business_id,restaurant_or_business")->find();
            session("USER",array(
                "id"=>$vipInfo['id'],
            ...
            // 判断是代理还是店铺
            if($vipInfo['restaurant_or_business'] == 1){
                // 代理
                session("business_id",$vipInfo['business_id']);
            }else{
                // 店铺
                session("restaurant_id",$vipInfo['restaurant_id']);
            }
            ...
            if($order_status == 0 && $pay_time == 0){
                ...
                if($vipInfo['restaurant_or_business'] == 1){
                    ...
                    $condition['restaurant_id'] = session("business_id");
                }
                // 手动添加类型条件
...
//        $User->where('order_status>=3 AND order_status<>10')->select();
    ...
    public function member_info()
    {
        ...
        if(IS_POST){
            ...
            $vipInfo = $vip->where(array("id"=>I("post.id")))->field("birthday,password")->find();
            if($vipInfo['password'] != I('post.password')){
                // 修改了密码
                $_POST['password'] = md5(I('post.password'));
            }
            $before_birthday = $vipInfo['birthday'];
    ...
    public function sms(){
        ...
        if($mobile){
            ...
            if(session('restaurant_flag')){
                // 店铺
                // 读取短信配置信息
                $sms_info = M("sms_vip_restaurant")->where(array("restaurant_id"=>session("restaurant_id")))->find();
            }else{
                // 代理
                // 读取短信配置信息
    ...
    public function mobileReg()
    {
        if(IS_POST){
           ...
           /* // 避免重复注册
            $if_have_reg = $Vip->where(array('openid'=>$_POST['openid']))->find();
            if($if_have_reg){
                // 设置session
                $openid = $_POST['openid'];
                session("USER",array(
            ...
            // 判断关联代理还是关联店铺
            if(session("restaurant_flag")){
                $_POST['restaurant_id'] = session("restaurant_id");
                $_POST['restaurant_or_business'] = 2;
            }else{
                $_POST['business_id'] = session("business_id");
                $_POST['restaurant_or_business'] = 1;
            }
