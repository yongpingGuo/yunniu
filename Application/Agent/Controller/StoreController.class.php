...
class StoreController extends Controller{
	public function store(){
		...
		$typeList = M('business')->where($condition)->field('type')->find();
		$type = $typeList['type'];
//		dump($type);
		$condition['status'] = 1;
		$p = I("param.page");
		...
		if($restaurant_name_key == ""){		
			...
			$Arrlist2 = $restaurant->where($condition)->field('restaurant_id,restaurant_name,address,telephone1,telephone2,city3,lat,lng')->page($p,$pageNum)->select();
		}else{
	...
	public function add_store(){
		...
		if($rel){
	        ...
	        $food_category_data['img_url'] = '/Public/images/defaultFoodCate.png';
            ...
            $food_data['food_img'] = './Application/Admin/Uploads/default/unupload.png';
...
//            后台默认立减：-2  -3  -5  -10元
            $reduce = [2,3,5,10];
            foreach($reduce as $val){//构建具体时间数组
                $ext_reduce[] = array(
                    'restaurant_id' => $rel,
                    'val' => $val,
            ...
            $rsl18 = M("order_or_food_discount")->addAll($ext_reduce);

            // 默认折扣：7  8  8.5  9折
            $discount = [7,8,8.5,9];
            foreach($discount as $val){//构建具体时间数组
                $ext_discount[] = array(
                    'restaurant_id' => $rel,
                    'val' => $val,
            ...
            $rsl19 = M("order_or_food_discount")->addAll($ext_discount);
            if($rsl18 === false || $rsl19 === false){
                $restaurant->rollback();
            }
