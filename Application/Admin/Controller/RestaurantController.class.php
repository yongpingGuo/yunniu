...
class RestaurantController extends Controller{
    ...
    public function index(){
        ...
        if(I("post.")){
            ...
            //从数据库中获取数据，渲染页面
            $restaurant_id = session('restaurant_id');
            $restaurant = $restaurant_model->where("restaurant_id = $restaurant_id")->field('restaurant_id,restaurant_name,telephone1,telephone2,address,logo,push_type,erp_card')->find();
            ...
            $_SESSION['vip_mode'] = $type['vip_mode'];//把类型存到session中
            $this->display('index');
        }
    }
