...
//        dump($group_info);
    ...
    public function save_group(){
        ...
        $group_names = $vip_group->where(array("restaurant_id"=>session("restaurant_id")))->field("group_name")->select();
        $arr = array();
    ...
    public function setting()
    {
        ...
        if (IS_POST) {
            ...
            /*********整单或者菜品打折信息*********/
            //折扣
            $discount = M('order_or_food_discount')->where(array('restaurant_id'=>session('restaurant_id'),'discount_or_reduce'=>1))->order('id')->select();
            // 立减
            $reduce = M('order_or_food_discount')->where(array('restaurant_id'=>session('restaurant_id'),'discount_or_reduce'=>2))->order('id')->select();
...
//    }
    ...
    public function remind_set(){
        ...
        $where = array("restaurant_id"=>$restaurant_id,"type"=>6);  // type值为6即会员余额
        $data = $set->where($where)->find();
        if($data){
            // 已有记录，就更新
            if($set->where($where)->save($receive) !== false){
    ...
    public function prepaid_set()
    {
        ...
        $restaurant_id = session("restaurant_id");
        $receive['restaurant_id'] = $restaurant_id;
        $where = array("restaurant_id"=>$restaurant_id,"type"=>0);  // 指定类型为$type
    ...
    public function add_prepaid(){
        ...
        $account['restaurant_id'] = session("restaurant_id");
        $account['account'] = I("post.account");
        $account['type'] = 1;

        $benefit['restaurant_id'] = session("restaurant_id");
    ...
    public function save_prepaid(){
        ...
        if(!$id || $id==$self_id){
            $save['restaurant_id'] = session("restaurant_id");
...
//        dump($condition2);
        $if_open = $set->where($condition2)->getField("if_open");
//        dump($if_open);
        $this->assign("if_open",$if_open);
        # 获取积分现金规则
        // 要根据business_id和类型去获取
...
//
//        // 积分商品
//        $goods_condition['restaurant_id'] = $restaurant_id;
//        $score_goods = D("score_goods");
...
//        die();
    ...
    public function get_public_number_set()
    {
        ...
        $restaurantData = $restaurantModel->field('restaurant_id,business_id,restaurant_name')->where($condition)->select();
        $this->assign("public_number_set",$public_number_set);
        $this->assign("business_id",$restaurantData['0']['business_id']);
        $this->assign("restaurant_id",$restaurant_id);
