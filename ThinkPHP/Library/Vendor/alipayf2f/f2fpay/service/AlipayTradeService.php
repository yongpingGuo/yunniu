...
class AlipayTradeService {
	...
	public function qrPay($req) {
        ...
        file_put_contents(__DIR__."/"."logsss.txt","result888".$response->code."\r\n",FILE_APPEND);
	...
	private function aopclientRequestExecute($request, $token = NULL, $appAuthToken = NULL) {
        ...
        file_put_contents(__DIR__."/"."logsss666.txt","private".$this->private_key."public_key".$this->alipay_public_key."\r\n",FILE_APPEND);
        file_put_contents(__DIR__."/"."logsss666.txt","format".$this->format."\r\n",FILE_APPEND);
		// 开启页面信息输出
		$aop->debugInfo=true;
		$result = $aop->execute($request,$token,$appAuthToken);
		//打开后，将url形式请求报文写入log文件
		//$this->writeLog("response: ".var_export($result,true));
        file_put_contents(__DIR__."/"."logsss666.txt","result".$result."\r\n",FILE_APPEND);
		return $result;
	}
